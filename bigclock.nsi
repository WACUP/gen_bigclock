...
!define VERSION "1.03"
!define ALT_VER "1_03"
!define PLUG "Big Clock"
!define PLUG_ALT "Big_Clock"
!define PLUG_FILE "gen_bigclock"
...
Section ""
  ...
  ; to avoid conflict with the original version
  ; we have a different filename and rename the
  ; original plug-in so it will not be loaded if
  ; found but so that it can be re-used if liked
  ${if} ${FileExists} "$INSTDIR\Plugins\gen_nxsbigclock.dll"
    Rename "$INSTDIR\Plugins\gen_nxsbigclock.dll" "$INSTDIR\Plugins\gen_nxsbigclock.original"
  ${endif}
SectionEnd
